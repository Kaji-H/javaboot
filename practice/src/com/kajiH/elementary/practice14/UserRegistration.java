package com.kajiH.elementary.practice14;
...
class UserList {
    ...
    public void addUser(User user) {
        if (!user.isValidFirstName()) {
            System.out.printf("ID %d: %s%s は登録できません. %n", user.id, user.firstName, user.lastName);
        ...
        System.out.printf("ID %d: %s%s を登録しました. %n%n", user.id, user.firstName, user.lastName);
        this.users.add(user);
    }
    ...
    public void showAllUsers() {
        ...
        for (User user : this.users) {
            System.out.printf("ID %d: %s%s %n", user.id, user.firstName, user.lastName);
        }
    }
}
