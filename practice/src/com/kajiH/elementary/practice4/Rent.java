package com.kajiH.elementary.practice4;

public class Rent {
    public static void main(String[] args) {
        Rent app = new Rent();
        RentalInfo yamamotoInfo = new RentalInfo("山本一郎", "aa", "bb", "10", "2000");
        RentalInfo yamadaInfo = new RentalInfo("山田太郎", "aa", "bb", "80", "2000");
        RentalInfo yamaguchiInfo = new RentalInfo("山口次郎", "aa", "bb", "15", "500");
    ...
    private void doRegistration(RentalInfo info) {
        System.out.println("-----");
        System.out.printf("%s さんの登録審査をします. %n", info.name);
        if (isValid(info)) {
            info.registerRentalProperty();
        } else {
            System.out.println("賃貸情報を登録できませんでした.");
        }
        System.out.println();
    }
    ...
    public boolean isValid(RentalInfo info) {
        ...
        if (!isNumeric(info.onFootMinutes)) {
            System.out.println("徒歩には数値を入力してください.");
            return false;
        }
        if (!isNumericRange(info.onFootMinutes, 0, 60)) {
        ...
        if (!isNumeric(info.monthlyRent)) {
            System.out.println("徒歩には数値を入力してください.");
            return false;
        }
        if (!isNumericRange(info.monthlyRent, 1000, 200000)) {
    ...
    public boolean isNumericRange(String target, int from, int to) {
...
class RentalInfo {
    ...
    public RentalInfo(String name, String wayside, String nearestStation,
            String onFootMinutes, String monthlyRent) {
        this.name = name;
